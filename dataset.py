...
import numpy as np
import pandas as pd
from config import spec_win_size_pcg, spec_win_size_ecg, outputpath, inputpath_physionet, inputpath_ephnogram_target, inputpath_ephnogram_data, nfft_pcg, nfft_ecg, load_config, useDrive
...
class ECGPCGDataset(Dataset):
    ...
    def __init__(self, pathtype="spec_and_mp4", path_videos_physionet=outputpath+f'physionet/spectrograms_{opts.ecg_type}/', path_audios_physionet=outputpath+f'physionet/spectrograms_{opts.pcg_type}/', path_audios_all=None, \
        ...
        self.path_audios = []
        self.path_videos_physionet = path_videos_physionet
        self.path_videos_ephnogram = path_videos_ephnogram
        self.path_audios_physionet = path_audios_physionet
        self.path_audios_ephnogram = path_audios_ephnogram
        ...
        self.parent_indexes = []
        self.pis = []
        # If the path_videos_all parameter is not supplied
        if not None == path_videos_all == path_audios_all == path_csv_all and None not in (path_videos_all, path_audios_all, path_csv_all):
            raise ValueError(f"Error: path_csv_all, path_videos_all and path_audios_all must all be supplied if one is supplied - csv must have columns {dataframe_cols}")
        if None not in (path_videos_all, path_audios_all, path_csv_all):
        ...
        if pathtype not in {"wav_and_mp4", "spec_and_mp4", "npy_and_mp4", "wav_and_wfdb", "mp4", "npy_and_npy"}:
            ...
            for i in range(2):
                if i == 0:
                    path_vs = self.path_videos_physionet
                    path_as = self.path_audios_physionet
                else:
                    path_vs = self.path_videos_ephnogram
                    path_as = self.path_audios_ephnogram
                dirs = next(os.walk(path_vs))[1]
                ddirs.extend(dirs)
                for d in dirs:
            ...
            self.zipped_indexes = list(zip(ddirs, self.pis))
    ...
    def __getitem__(self, index, normalise=True, colname='spec', print_short=False, inputpath_training_p=inputpath_physionet, inputpath_training_e=inputpath_ephnogram_data, inputpath_target_e=inputpath_ephnogram_target, outputpath_=outputpath):
        ...
        if vext == ".mp4":
            ...
            if index_of_parent < 409:  
                ...
                seg_spectrogram = Spectrogram(filename, savename=ecg.savename, filepath=outputpath_+'physionet/', sample_rate=ecg_sample_rate, type=opts.ecg_type,
                                                signal=ecg.signal, window=np.hamming, window_size=spec_win_size_ecg, NFFT=nfft_ecg, hop_length=spec_win_size_ecg//2, 
                                                outpath_np=outputpath_+f'physionet/data_{opts.ecg_type}/{filename}/{index_e}/', outpath_png=outputpath_+f'physionet/spectrograms_{opts.ecg_type}/{filename}/{index_e}/', normalise=True, start_time=ecg.start_time, wavelet_function=opts.cwt_function)
                #specs.append(seg_spectrogram)
                seg_spectrogram.display_spectrogram(save=True)
                
                frames = []
                print(f"* Processing Frames for Segment {index_e} *")
                ecg_frames = ecg.get_segments(opts.frame_length, factor=opts.fps*opts.frame_length, normalise=False) #24fps, 42ms between frames, 8ms window, 128/8=16
                ...
                ecg_seg_video = create_video(imagespath=outputpath_+f'physionet/spectrograms_{opts.ecg_type}/{filename}/{index_e}/frames/', outpath=outputpath_+f'physionet/spectrograms_{opts.ecg_type}/{filename}/{index_e}/', filename=ecg.savename, framerate=opts.fps)
            else:  
                ...
                ecg_segments = ecg.get_segments(opts.segment_length, normalise=True)
                ecg = ecg_segments[index_of_segment]
                seg_spectrogram = Spectrogram(filename, savename=ecg.savename, filepath=outputpath_+'ephnogram/', sample_rate=ecg_sample_rate, type=opts.ecg_type,
                                              signal=ecg.signal, window=np.hamming, window_size=spec_win_size_ecg, NFFT=nfft_ecg, hop_length=spec_win_size_ecg//2, 
                                              outpath_np=outputpath_+f'ephnogram/data_{opts.ecg_type}/{filename}/{index_e}/', outpath_png=outputpath_+f'ephnogram/spectrograms_{opts.ecg_type}/{filename}/{index_e}/', normalise=True, start_time=ecg.start_time, wavelet_function=opts.cwt_function)
                #specs.append(seg_spectrogram)
                seg_spectrogram.display_spectrogram(save=True)
        ...
        if aext == ".wav":
            ...
            if index_of_parent < 409:
                ...
                pcg_seg_spectrogram = Spectrogram(filename, savename=pcg.savename, filepath=outputpath_+'physionet/', sample_rate=pcg_sample_rate, type=opts.pcg_type,
                                      ...
                                      outpath_np=outputpath_+f'physionet/data_{opts.pcg_type}/{filename}/{index_p}/', outpath_png=outputpath_+f'physionet/spectrograms_{opts.pcg_type}/{filename}/{index_p}/', normalise=True, start_time=pcg.start_time)
                ...
                pcg_segments = pcg.get_segments(opts.segment_length, normalise=True)
                pcg = pcg_segments[index_of_segment]
                pcg_seg_spectrogram = Spectrogram(filename, savename=pcg.savename, filepath=outputpath_+'ephnogram/', sample_rate=pcg_sample_rate, type=opts.pcg_type,
                                    signal=pcg.signal, window=torch.hamming_window, window_size=spec_win_size_pcg, NFFT=nfft_pcg, hop_length=spec_win_size_pcg//2, NMels=opts.nmels,
                                    outpath_np=outputpath_+f'ephnogram/data_{opts.pcg_type}/{filename}/{index_p}/', outpath_png=outputpath_+f'ephnogram/spectrograms_{opts.pcg_type}/{filename}/{index_p}/', normalise=True, start_time=pcg.start_time)
        ...
        assert sr_a == opts.sample_rate_ecg
        if not opts.fps == sr_v:
            print(f"Warning: specified fps ({opts.fps}) is different from video fps ({sr_v}); resampling to {opts.fps}fps")
            resample_video(video_path, opts.fps)
